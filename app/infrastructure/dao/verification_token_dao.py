...
class VerificationTokenDAO:
    ...
    async def get_by_token(self, *, hashed_token: str) -> VerificationTokenModel | None:
        """Retrieve a verification token by its token string."""

        statement = select(VerificationTokenModel).where(VerificationTokenModel.hashed_token == hashed_token)
